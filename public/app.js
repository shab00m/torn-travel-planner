...
function compareItemsBySort(aCountry, aItem, bCountry, bItem, sortState) {
	...
	switch (column) {
		...
		case "country":
			return compareSortValues(
				countrySortLabel(aCountry),
				countrySortLabel(bCountry),
				dir,
			);
		case "type":
			return compareSortValues(
...
function getSortedFavorites() {
	...
	if (!state.showHidden) {
		favorites = favorites.filter(
			({ country, item }) => !isHidden(country, item.id),
		);
	}
...
function populateItemTypeFilter() {
	...
	if (state.itemTypesStatus === "ready" && state.stocks) {
		const pruned = state.itemTypeFilters.filter((type) =>
	...
	const extras = state.itemTypeFilters.filter(
		(type) => !available.includes(type),
	);
	const options = [...available, ...extras].map((type) => ({
		value: type,
		label: type,
	}));
	const typeLabels = {
	...
	renderCheckboxOptions(
		el.itemTypeFilterList,
		options,
		state.itemTypeFilters,
		(value, checked) => {
...
function profitHrCell(country, item) {
	...
	if (profitPerHour != null) {
		...
		return `<td class="profit-hr ${cls}"${title ? ` title="${escapeHtml(title)}"` : ""}>${fmtListMoneyHtml(Math.round(profitPerHour))}/hr</td>`;
	}
	const title =
		getSellPrice(country, item.id) == null &&
		state.marketPricesStatus === "empty"
...
async function loadMarketPrices() {
	try {
		...
		state.marketPricesStatus = Object.keys(state.marketPrices).length
			? "ready"
			: "empty";
	} catch (err) {
...
async function loadItemTypes() {
	try {
		...
		state.itemTypesStatus = Object.keys(state.itemTypes).length
			? "ready"
			: "empty";
	} catch {
...
async function loadSafeWindows() {
	...
	try {
		...
		state.safeWindowsStatus = Object.keys(state.safeWindows).length
			? "ready"
			: "error";
	}
	renderFavoritesOnly();
...
function favoritesCardHtml(favorites) {
	const rows = favorites
		.map(({ country, item }) => favoriteRowHtml(country, item))
		.join("");
	return `
...
function renderFavoritesOnly() {
	...
	const header = existingCard.querySelector(
		".itemlist-header .country-updated",
	);
	if (header) {
...
function renderFavoritesSection() {
	if (!el.favorites) return;
	const favorites = getSortedFavorites();
	el.favorites.innerHTML = favorites.length
		? favoritesCardHtml(favorites)
		: "";
}
...
function collectStockRows(query) {
	...
	for (const code of Object.keys(state.countries)) {
		if (state.countryFilters.length && !state.countryFilters.includes(code))
			continue;
...
function buildStockGroups(rows, query) {
	...
	for (const [code, meta] of Object.entries(state.countries)) {
		...
		if (
			!groupRows.length &&
			(data.stocks.length > 0 || hasActiveListFilters(query))
		)
			continue;
		groups.push({
			key: code,
...
function stockGroupCardHtml(group) {
	...
	const body = group.rows
		.map(({ country, item }) => stockRowHtml(country, item))
		.join("");
	card.insertAdjacentHTML(
		"beforeend",
		`<table>
...
function render() {
	...
	el.countries.classList.toggle(
		"countries-flat",
		state.stockGroupBy === "none",
	);
...
function syncProfitRangeFromInputs() {
	...
	if (
		state.profitMin != null &&
		state.profitMax != null &&
		state.profitMin > state.profitMax
	) {
...
el.homeMain.addEventListener("click", async (e) => {
	...
	if (sortBtn) {
		...
		if (
			target === "favorites" &&
			el.favorites?.querySelector(".favorites-card")
		) {
			renderFavoritesOnly();
		} else {
			render();
