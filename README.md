...
- Depletion rate per in-stock window (restock -> next depletion, or -> now while stock lasts) in items/minute, with the same sample-average selector and dashed trend lines overlaid on the graph
...
`npm run backfill` replays all stored snapshots through the restock detection logic. It is idempotent and only needed if snapshots were collected without restock tracking (or to sanity-check the data).
